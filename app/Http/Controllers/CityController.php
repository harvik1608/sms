...
class CityController extends Controller
{
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                $actions = '<div class="edit-delete-action">';
                    $actions .= '<a href="' . url('cities/'.$row->id.'/edit/') . '" class="me-2 edit-icon p-2 text-success" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                        </svg>
                    </a>';
                    $actions .= '<a href="javascript:;" onclick="remove_row(\'' . url('cities/' . $row->id) . '\')" data-bs-toggle="modal" data-bs-target="#delete-modal" class="p-2" title="Delete">
    ...
    public function create()
    {
        ...
        return view('city.add_edit',compact('city','states'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $row->state_id = trim($post['state_id']);
    ...
    public function update(Request $request,$id)
    {
        try {
            ...
            return response()->json(['success' => true,'message' => "City edited successfully."], 200);
        } catch (\Exception $e) {
            return response()->json(['success' => false,'message' => $e->getMessage()], 200);
        }
    }
    ...
    {
        City::destroy($id);
        return response()->json(['success' => true,'message' => "City removed successfully."], 200);
    }
}
